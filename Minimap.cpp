...
{
    for (int i = 0; i < 3; ++i)  for (int j = 0; j < 3; ++j) roomSquare[i][j] = 0;
    for (int i = 0; i < 18; ++i) pathRectangle[i] = 0;
...
Minimap::~Minimap() {
    for (auto& shape : squares) delete shape;
}
...
void Minimap::render(RenderTarget* target) {
    for (auto& shape : squares) target->draw(*shape);
}
